...
rtl.module("Rtl.ScriptLoader",["System","Types"],function () {
  "use strict";
  var $mod = this;
  this.loadScripts = function (scripts, callback, Data) {
    function loader(src, handler) {
      ...
      script["onreadystatechange"] = DoLoaded;
      head = document.getElementsByTagName("head").item(0);
      if (head === null) head = document.body;
      head.appendChild(script);
    };
    function run() {
      if (rtl.length(scripts) !== 0) {
        loader("" + scripts.shift(),run)}
       else if (callback != null) callback(Data);
    };
    run();
  };
},["JS","Web"]);
...
rtl.module("Rtl.UnitLoader",["System","SysUtils","JS","Types"],function () {
  "use strict";
  var $mod = this;
  var $impl = $mod.$impl;
  rtl.createClass(this,"EUnitLoader",pas.SysUtils.Exception,function () {
  });
  rtl.createClass(this,"TLoadTask",pas.System.TObject,function () {
    this.$init = function () {
      pas.System.TObject.$init.call(this);
      this.FUnitNames = [];
      this.FInitUnitNames = [];
    ...
    this.$final = function () {
      this.FUnitNames = undefined;
      this.FInitUnitNames = undefined;
      this.FOnLoaded = undefined;
      this.FData = undefined;
      pas.System.TObject.$final.call(this);
    };
    this.GetAllLoaded = function () {
      var Result = false;
      Result = rtl.length(this.FInitUnitNames) === 0;
      return Result;
    };
    this.CallLoaded = function () {
      if (this.FOnLoaded != null) this.FOnLoaded(this.FUnitNames,this.FData);
    };
    this.Create$1 = function (aUnitNames, aOnLoaded, aData) {
      var I = 0;
      this.FUnitNames = rtl.arraySetLength(this.FUnitNames,"",rtl.length(aUnitNames));
      this.FInitUnitNames = rtl.arraySetLength(this.FInitUnitNames,"",rtl.length(aUnitNames));
      for (var $l = 0, $end = rtl.length(aUnitNames) - 1; $l <= $end; $l++) {
        I = $l;
        this.FUnitNames[I] = aUnitNames[I];
        this.FInitUnitNames[I] = aUnitNames[I];
      };
      this.FOnLoaded = aOnLoaded;
      this.FData = aData;
      return this;
    };
    this.UnitLoaded = function (aUnitName) {
      var Idx = 0;
      Idx = $impl.IndexOfI(rtl.arrayRef(this.FInitUnitNames),aUnitName);
      if (Idx > -1) this.FInitUnitNames.splice(Idx,1);
    };
  });
  rtl.createClass(this,"TUnitLoader",pas.System.TObject,function () {
    this.FInstance = null;
    this.$init = function () {
      pas.System.TObject.$init.call(this);
      this.FBaseURL = "";
    ...
    this.$final = function () {
      this.FLoadList = undefined;
      pas.System.TObject.$final.call(this);
    };
    this.DoDependenciesLoaded = function (aUnitName, aData) {
      this.UnitSourcesLoaded(aData);
    };
    this.IndexOfLoadUnit = function (aUnitName) {
      var Result = 0;
      Result = $impl.IndexOfI(rtl.arrayRef(this.FLoadList),aUnitName);
      return Result;
    };
    this.AddToLoadList = function (aUnitName) {
      if (this.IndexOfLoadUnit(aUnitName) < 0) this.FLoadList.push(aUnitName);
    };
    this.RemoveFromLoadList = function (aUnitName) {
      var idx = 0;
      idx = this.IndexOfLoadUnit(aUnitName);
      if (idx > -1) this.FLoadList.splice(idx,1);
    };
    this.IsInLoadList = function (aUnitName) {
      var Result = false;
      Result = this.IndexOfLoadUnit(aUnitName) >= 0;
      return Result;
    };
    this.GetUnitURL = function (aUnitName) {
      var Result = "";
    ...
    this.InitModule = function (aTask, aName, aModule) {
      this.RemoveFromLoadList(aName);
      rtl.loadintf(aModule);
      rtl.loadimpl(aModule);
      aTask.UnitLoaded(aName);
    };
    this.DoLoadUnits = function (aUnitNames, aOnLoaded, aData) {
      var Scripts = [];
      var aCount = 0;
      var S = "";
      aCount = 0;
      Scripts = rtl.arraySetLength(Scripts,"",rtl.length(aUnitNames));
      for (var $in = aUnitNames, $l = 0, $end = rtl.length($in) - 1; $l <= $end; $l++) {
        S = $in[$l];
        if (!this.HaveModule(S)) {
          Scripts[aCount] = this.GetUnitURL(S);
          this.AddToLoadList(S);
          aCount += 1;
        };
      };
      S = rtl.strSetLength(S,aCount);
      if (aCount === 0) {
        if (aOnLoaded != null) aOnLoaded(aUnitNames,aData);
      } else pas["Rtl.ScriptLoader"].loadScripts(rtl.arrayRef(Scripts),rtl.createCallback(this,"UnitSourcesLoaded"),$mod.TLoadTask.$create("Create$1",[aUnitNames,aOnLoaded,aData]));
    };
    this.GetNeededDependencies = function (aName, AModule) {
      var Result = [];
      var l = [];
      var u = [];
      var m = "";
      l = rtl.arraySetLength(l,"",0);
      u = AModule["$intfuseslist"];
      for (var $in = u, $l = 0, $end = rtl.length($in) - 1; $l <= $end; $l++) {
        m = $in[$l];
        if (!(this.HaveModule(m) || this.IsInLoadList(m))) l.push(m);
      };
      u = AModule["$impluseslist"];
      for (var $in1 = u, $l1 = 0, $end1 = rtl.length($in1) - 1; $l1 <= $end1; $l1++) {
        m = $in1[$l1];
        if (!(this.HaveModule(m) || this.IsInLoadList(m))) l.push(m);
      };
      Result = rtl.arrayRef(l);
      return Result;
    };
    this.UnitSourcesLoaded = function (aData) {
      var aTask = null;
      var aModule = undefined;
      var aModuleName = "";
      var Deps = [];
      aTask = aData;
      for (var $in = aTask.FUnitNames, $l = 0, $end = rtl.length($in) - 1; $l <= $end; $l++) {
        aModuleName = $in[$l];
        aModule = this.FindModule(aModuleName);
        if (aModule != null) {
          Deps = this.GetNeededDependencies(aModuleName,aModule);
          if (rtl.length(Deps) === 0) {
            this.InitModule(aTask,aModuleName,aModule)}
           else this.DoLoadUnits(Deps,rtl.createCallback(this,"DoDependenciesLoaded"),aData);
        };
      };
      if (aTask.GetAllLoaded()) aTask.CallLoaded();
    };
    this.Instance = function () {
      var Result = null;
      if (this.FInstance === null) $mod.TUnitLoader.FInstance = $mod.TUnitLoader.$create("Create");
      Result = this.FInstance;
      return Result;
    };
    this.FindModule = function (aModuleName) {
      var Result = undefined;
      var Key = "";
      Result = pas[aModuleName];
      if (rtl.isModule(Result)) return Result;
      for (Key in pas) {
        if (!pas.SysUtils.SameText(Key,aModuleName)) continue;
        Result = pas[Key];
        if (rtl.isModule(Result)) return Result;
      };
      Result = null;
    ...
    this.HaveModule = function (aModuleName) {
      var Result = false;
      Result = this.FindModule(aModuleName) != null;
      return Result;
    };
    this.LoadUnit = function (aUnitName, aOnLoaded, aData) {
      this.LoadUnits([aUnitName],aOnLoaded,aData);
    };
    this.LoadUnits = function (aUnitNames, aOnLoaded, aData) {
      if (rtl.length(this.FLoadList) > 0) throw $mod.EUnitLoader.$create("Create$1",["Load operation in progress. Cannot load."]);
      this.DoLoadUnits(aUnitNames,aOnLoaded,aData);
    };
  });
  $mod.$implcode = function () {
    $impl.IndexOfI = function (arr, Name) {
      var Result = 0;
      Result = rtl.length(arr) - 1;
      while ((Result >= 0) && !pas.SysUtils.SameText(arr[Result],Name)) Result -= 1;
      return Result;
    };
  };
},["Rtl.ScriptLoader"]);
...
rtl.module("formfactory",["System","Classes"],function () {
  "use strict";
  var $mod = this;
  var $impl = $mod.$impl;
  this.RegisterForm = function (aClass) {
    var Def = $impl.TFormDef.$new();
    Def.aClass = aClass;
    Def.aName = aClass.$classname;
    $impl.List.push(Def);
  };
  this.GetFormClassByName = function (aName) {
    var Result = null;
    var Idx = 0;
    Idx = $impl.IndexOfFormClassByName(aName);
    if (Idx === -1) {
      Result = null}
     else Result = $impl.List[Idx].aClass;
    return Result;
  };
  $mod.$implcode = function () {
    rtl.recNewT($impl,"TFormDef",function () {
      this.aClass = null;
      this.aName = "";
      this.$eq = function (b) {
        return (this.aClass === b.aClass) && (this.aName === b.aName);
      };
      this.$assign = function (s) {
        this.aClass = s.aClass;
        this.aName = s.aName;
        return this;
      };
    });
    $impl.List = [];
    $impl.IndexOfFormClassByName = function (aName) {
      var Result = 0;
      Result = rtl.length($impl.List) - 1;
      while ((Result >= 0) && !pas.SysUtils.SameText(aName,$impl.List[Result].aName)) Result -= 1;
      return Result;
    };
  };
  $mod.$init = function () {
    $impl.List = rtl.arraySetLength($impl.List,$impl.TFormDef,0);
  };
},["SysUtils","JS"]);
rtl.module("program",["System","Types","JS","Web","Rtl.ScriptLoader","Rtl.UnitLoader","formfactory"],function () {
  "use strict";
  var $mod = this;
  this.DoFormClick = function (e) {
    var Result = false;
    function DoLoaded(aUnitName, aData) {
      var C = null;
      if (C !== null) pas.formfactory.RegisterForm(C);
      pas.System.Writeln("Unit ",aUnitName," was loaded");
      C = pas.formfactory.GetFormClassByName("TForm1");
      if (C !== null) {
        C.$create("Create$1",[null])}
       else {
        pas.System.Writeln("TMyForm not found");
      };
      ...
      pas.layui.layui_open();
     	...
     	pas["Rtl.UnitLoader"].TUnitLoader.Instance().LoadUnit("unit1frm",DoLoaded,null); 
    Result = false;
    return Result;
  };
  $mod.$main = function () {
    document.getElementById("formloader").onclick = rtl.createSafeCallback($mod,"DoFormClick");
  };
});
